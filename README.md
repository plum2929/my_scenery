...
## ■サービス概要
「日々の生活や旅先で目にした素敵な景色をどこかの誰かと共有したい。だけどSNSは疲れてしまうから苦手。」そんな方に向けた、風景写真の投稿・観覧・検索ができるサービスです。
...
1. ユーザー同士の交流をあえてなくすことで、「人からどう思われるか」ではなく「自分はどう思うか」に集中して利用できるサービスを提供する。
    ...
    - 「気になる」に保存された数も同様に開示しない。
2. 気に入った写真の場所に足を運べるよう、撮影場所を地図で確認できるようにする。
    - 写真の投稿には必要最低限の情報(タグ、撮影場所)のみ登録できるようにする。
3. 気になった写真が自動的に保存されるようにすることで、「自分がどう思っているのか」を可視化できるようにする。
    - 詳細画面を観覧した投稿は自動的に「気になる」に保存されるようする。
    - 「気になる」では自分が表示した回数の多い投稿ほど上位に表示されるようにする。
...
## ■実装予定の機能
### 未ログインユーザー
- 投稿された風景写真を観覧できる（遅延読み込み機能を使用）
    - 検索バーからタグと撮影場所による検索ができる
- 投稿の詳細ページでは風景写真、タグ、撮影場所、撮影場所の地図を観覧できる
- ユーザーの詳細ページではそのユーザーが投稿した風景写真の一覧を観覧できる
- ユーザーの新規登録ができる
- 登録済みユーザーがログインできる
    - メールアドレスでログインできる
    - Googleアカウントでログインできる
...
- 風景写真の投稿・編集・削除ができる
    ...
    - 投稿・編集時に撮影場所とその地図を追加できる
        - 地図の登録には「現在地から検索」と「スポット名から検索」を利用できる
- 自分の投稿や他のユーザーの投稿をアルバムに保存できる
    - アルバムは複数作成できる
    - １つの投稿を複数のアルバムに保存できる
- 他のユーザーをフォローできる
    - フォローしたユーザーの投稿のみを集めたページを観覧できる
    - フォローしたユーザーが新規投稿した場合、アプリ内で通知される
- 詳細を観覧した投稿は自動的に「気になる」に保存される
    - 「気になる」では自分が表示した回数の多い投稿ほど上位に表示される
